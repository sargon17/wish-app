...
import {
  ...
  formatWeekLabel,
  startOfWeekUTC,
} from "../../../../packages/convex-backend/convex/lib/requestOverviewReadModel.ts";
...
describe("requestOverviewReadModel", () => {
  ...
  it("keeps owned requests with missing project documents and falls back to unknown project labels", () => {
    ...
    expect(overview.weeklyTrend.at(-1)).not.toBeUndefined();
    expect(overview.weeklyTrend.at(-1)?.count).toBe(1);
  });
  ...
  it("sorts breakdowns by descending count and deterministic tie order", () => {
    ...
    expect(overview.statusBreakdown.map((item) => item.statusId)).toEqual([
      statusOpen,
      statusClosed,
      statusQueued,
    ]);
